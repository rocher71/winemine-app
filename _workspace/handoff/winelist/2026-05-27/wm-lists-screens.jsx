...
// ════════════════════════════════════════════════════════════════
// SCREEN 2A — 내 리스트 상세 (owner view)
// ════════════════════════════════════════════════════════════════
function MyListWineRow({ wine, idx }) {
  return (
    <div style={{
      ...
      {/* index — tasted gets a check */}
      <div style={{
        minWidth: 22, display: 'flex', justifyContent: 'flex-end', alignItems: 'center',
      }}>
        {wine.tasted ? (
          <div style={{
          ...
          <span style={{
            ...
            color: WT.faint, fontWeight: 500, fontVariantNumeric: 'lnum',
          }}>{String(idx + 1).padStart(2,'0')}</span>
        )}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          fontFamily: WT.body, fontSize: 13.5,
          color: wine.tasted ? WT.muted : WT.ink,
          fontWeight: 700, letterSpacing: '-0.005em', lineHeight: 1.3,
          textDecoration: wine.tasted ? 'line-through' : 'none',
          textDecorationColor: WT.faint,
        }}>{wine.name}</div>
        <div style={{ fontSize: 11, color: WT.muted, marginTop: 2, lineHeight: 1.4 }}>
          {wine.producer} · {wine.vintage} · <i>{wine.region}</i>
          {wine.note && (
            <>
              <span style={{ display: 'inline-block', width: 2, height: 2, borderRadius: 999, background: WT.faint, margin: '2px 6px', verticalAlign: 'middle' }}/>
              <span style={{ color: WT.goldSoft, fontWeight: 600 }}>{wine.note}</span>
            </>
          )}
        </div>
      </div>
      <button style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}><Icon name="more" size={15} color={WT.muted}/></button>
    </div>
  );
}

function ScreenMyListDetail() {
  const list = MY_LIST_DETAIL;
  const isPublic = list.visibility === 'public';
  const tasted = list.wines.filter(w => w.tasted).length;
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
      <WTStatusBar time="5:45"/>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '4px 24px 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button style={{
              ...
              background: isPublic ? `${WT.goldSoft}18` : WT.cardSoft,
              border: `1px solid ${isPublic ? `${WT.goldSoft}55` : WT.border}`,
              color: isPublic ? WT.goldSoft : WT.ink2,
              fontFamily: WT.body, fontSize: 11, fontWeight: 700,
              letterSpacing: '-0.005em', cursor: 'pointer',
            }}>
              <Icon name={isPublic ? 'globe' : 'lock'} size={11}
                color={isPublic ? WT.goldSoft : WT.ink2} sw={2}/>
              {isPublic ? '공개' : '비공개'}
            </button>
          </div>

          {/* Title — editable hint with subtle pencil */}
          <div style={{
            marginTop: 8, display: 'flex', alignItems: 'flex-start', gap: 8,
          }}>
            <div style={{
              flex: 1,
              fontFamily: WT.display, fontSize: 30, color: WT.ink,
              fontWeight: 600, letterSpacing: '-0.02em', lineHeight: 1.1,
              fontStyle: 'italic',
            }}>{list.title}</div>
            <button style={{
              width: 30, height: 30, borderRadius: 999, border: 'none',
              background: WT.cardSoft, cursor: 'pointer', padding: 0,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              marginTop: 4,
            }}><Icon name="edit" size={13} color={WT.ink2} sw={1.9}/></button>
          </div>
          ...
          <div style={{
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              <b style={{ color: WT.ink, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{list.count}</b>
              <span>병</span>
            </span>
            <span style={{ width: 2, height: 2, borderRadius: 999, background: WT.faint }}/>
            <span>{list.createdAt} 시작</span>
            <span style={{ width: 2, height: 2, borderRadius: 999, background: WT.faint }}/>
            <span>{list.updated} 수정</span>
          </div>

          {/* Progress strip — tasted vs remaining */}
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                <span style={{
                  fontFamily: WT.body, fontSize: 22, color: WT.ink,
                  fontWeight: 700, letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums',
                }}>{tasted}<span style={{ fontSize: 14, color: WT.muted, fontWeight: 600 }}> / {list.count}</span></span>
                <span style={{ fontSize: 11, color: WT.muted, fontWeight: 500 }}>마신 와인</span>
              </div>
              {/* progress bar */}
              <div style={{
                marginTop: 10, height: 6, borderRadius: 999,
                background: WT.borderSoft, overflow: 'hidden', position: 'relative',
              }}>
                <div style={{
                  position: 'absolute', left: 0, top: 0, bottom: 0,
                  width: `${(tasted / list.count) * 100}%`,
                  background: `linear-gradient(90deg, ${WT.goldChip}, ${WT.goldSoft})`,
                  borderRadius: 999,
                }}/>
              </div>
            </div>
        ...
        <div style={{
          ...
          <span style={{
            fontSize: 10.5, color: WT.goldSoft, letterSpacing: '0.18em',
            textTransform: 'uppercase', fontWeight: 700,
          }}>Wines</span>
          <span style={{ fontFamily: WT.body, fontSize: 13, color: WT.ink, fontWeight: 700 }}>와인 {list.count}</span>
          <span style={{ flex: 1 }}/>
          <span style={{ fontSize: 11, color: WT.ink2, fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 3 }}>
            <Icon name="grip" size={11} color={WT.ink2} sw={1.8}/>
            순서 편집
          </span>
        </div>

        <div style={{
          flex: 1, padding: '0 24px 120px', overflow: 'hidden',
        }}>
          {list.wines.slice(0, 5).map((w, i) => <MyListWineRow key={i} wine={w} idx={i}/>)}
        </div>
      </div>
      ...
      <div style={{
        ...
        <button style={{
          ...
          <Icon name="edit" size={15} color={WT.ink} sw={1.9}/>
          편집
        </button>
        <button style={{
          ...
          <Icon name="plus" size={16} color="#FFFFFF" sw={2.4}/>
          와인 추가
        </button>
      </div>
    </div>
...
Object.assign(window, {
  ...
  ScreenLists, ScreenListDetail, ScreenMyListDetail, ScreenListCreate,
