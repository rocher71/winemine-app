...
0. **디자인 시각 게이트 통과 확인 (PREREQUISITE)** — design-reviewer PASS 보고서 존재 필수
...
화면 단위 incremental QA를 요청받았다면, **먼저** design-reviewer의 PASS 보고서 존재를 확인:

```bash
ls _workspace/design-review_{route}_*.md 2>/dev/null | tail -1
# 최신 보고서가 PASS인지 grep
rg -l "결과: PASS" _workspace/design-review_{route}_*.md
```

체크:
- 해당 라우트의 design-reviewer 보고서 존재
- 가장 최근 보고서의 "결정 → 결과: PASS"
- design-spec-author 사양도 존재 (`_workspace/design-specs/{route}.md`)

**없거나 FAIL이면**:
- rn-screen-builder + design-reviewer에 SendMessage: "QA 진행 불가 — design-reviewer 게이트 미통과"
- 이번 모듈 QA SKIP, 통과 후 재요청 받기
- 보고서에 "디자인 게이트 미통과 — SKIPPED" 1줄 기록

이유: 시각 게이트가 텍스트 기반 QA보다 먼저여야 디자인 갭이 누적되지 않음. design-reviewer는 6항목 시각 체크 (요소 누락·spacing·gradient·radius·typography·color)를 담당, qa-inspector는 데이터·로직 경계면을 담당.
